...
Isla::Isla( AdministradorArchivo *administrador )
{
	...
	pasto = new Pasto ( ptos, administrador->getNombreArchivo( PASTO ), paso);
	supBorde = new BordeIsla( ptos, administrador->getNombreArchivo( SUELO ), paso, cantNivelesHorizontalesIsla );
	playaBorde = new BordeIsla ( ptos, administrador->getNombreArchivo( ARENA ), paso, cantNivelesHorizontalesPlaya); // es el costado de la isla
	arena = new Arena ( ptos, administrador->getNombreArchivo( ARENA ), paso);
